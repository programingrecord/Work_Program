...
@interface MSUTestFiveController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MSUTestFiveController
...
- (void)viewDidLoad {
    ...
    self.dataArr = @[@"A.小于10%",@"B.10%-25%",@"C.25%-50%",@"D.大于50%"];
...
- (void)createCenterUI{
    ...
    titLab.attributedText = [MSUStringTools makeKeyWordAttributedWithSubText:@"/13" inOrigiText:@"5/13" font:12 color:HEXCOLOR(0xFFDCD2)];
    [self.view addSubview:titLab];
    
    UILabel *quesLab = [[UILabel alloc] init];
    quesLab.text = @"在您每年的家庭可支配收入中，可用于金融投资（储蓄存款除外）的比例为?";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MSUTestSixController *second = [[MSUTestSixController alloc] init];
    second.answerCode = indexPath.row + 1;
