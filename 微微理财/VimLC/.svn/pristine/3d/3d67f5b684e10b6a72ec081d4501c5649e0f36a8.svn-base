...
static CGFloat btnBigImageWidth   = 45;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.backgroundColor = [[UIColor colorWithHex:0xff967d] colorWithAlphaComponent:0];
//        [self setImage:[self resizeImage:[UIImage imageNamed:@"suspend-button"] wantSize:CGSizeMake(btnBigImageWidth, btnBigImageWidth)] forState:0];
...
- (void)setImaStr:(NSString *)imaStr{
    _imaStr = imaStr;

    [self sd_setImageWithURL:[NSURL URLWithString:_imaStr] forState:UIControlStateNormal placeholderImage:[self resizeImage:[UIImage imageNamed:@"suspend-button"] wantSize:CGSizeMake(btnBigImageWidth, btnBigImageWidth)]];
}
...
//    MyLog(@"%f  %f",point.x,point.y);
    ...
    switch (pan.state) {
        case UIGestureRecognizerStateEnded:
            ...
            if (point.y > KScreenHeight - fullButtonWidth / 2-64*kDeviceHeightScale) {
                [UIView animateWithDuration:0.5 animations:^{
                    self.lhz_y = KScreenHeight - fullButtonWidth-64*kDeviceHeightScale;
                }];
            }
...
- (void)suspendBallShow
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(suspendBallDidSelct)]) {
        [self.delegate suspendBallDidSelct];
    }
