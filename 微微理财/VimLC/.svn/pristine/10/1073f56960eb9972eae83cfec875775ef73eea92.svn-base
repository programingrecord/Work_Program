...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if(style==1){
        [rerificationDelegate verification:resultString];
    }
    else {
        [resetDelegate resetPassword:resultString];
    }
    [self setNeedsDisplay];
}
...
- (void)changeButtonColor:(NSNotification *)notice{
    NSDictionary *nameDictionary = [notice userInfo];
    NSString *isError = [nameDictionary objectForKey:@"isError"];
    if (isError && [isError isEqualToString:@"Y"]) {
        success = YES;
    }else{
        success = NO;
    }
...
- (void)drawRect:(CGRect)rect
{
    for (int i=0; i<touchesArray.count; i++) {
        ...
        if (success) {
            CGContextSetRGBStrokeColor(context, 255/255.f, 127/255.f, 39/255.f, 0.7);//线条颜色
        }
        else {
            CGContextSetRGBStrokeColor(context, 255/255.f, 60/255.f, 0/255.f, 0.6);//红色
        }
